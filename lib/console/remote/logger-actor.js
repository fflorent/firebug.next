...
const { ActorPool } = devtools["require"]("devtools/server/main");
...
/** @lends LoggerActor */
{
  ...
  initialize: function(conn, parent) {
    ...
    this._actorPool = new ActorPool(conn);
  ...
  sendMessage: function(msg) {
    ...
    let message = {
      ...
      private: false,
      timeStamp: Date.now(),
      arguments: prepareForRemote(formatted.logs, this.parent, this._actorPool),
      styles: formatted.styles,
    };

    // Send raw data to the client {@LoggerFront}
    // this.conn.send(packet);
    emit(this, "http-log", message);
...
// Helpers taken from devtools/server/actors/webconsole.js.
//
// xxxFlorent: Should be put in a Utils file in the platform.

function prepareForRemote(args, parentActor, actorPool) {
  let threadActor = parentActor.threadActor;
  return Array.map(args || [], (arg) => {
    let dbgValue = makeDebuggeeValue(arg, threadActor, parentActor.window);
    return threadActor.createValueGrip(dbgValue, actorPool);
  });
}

function makeDebuggeeValue(value, threadActor, global)
{
  if (typeof value == "object") {
    try {
      global = Cu.getGlobalForObject(value);
    }
    catch (ex) {
      // The above can throw an exception if value is not an actual object.
    }
  }
  let dbgGlobal = threadActor.dbg.makeGlobalObjectReference(global);
